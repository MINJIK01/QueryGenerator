...
#     token_ids = torch.argmax(logits_tensor, dim=-1)
#     special_tokens = [logit for token in token_ids if token in special_tokens]

#     decoded_texts = [tokenizer.decode(ids) for ids in token_ids]

#     # Print the results
#     for i, text in enumerate(decoded_texts):
#         print(f"Decoded Text for Sequence {i}: {text}")
...
def preprocess_function(examples, tokenizer, args):
    ...
    print(formmated_path)
    exit(1)
    # input_ids = labels['input_ids']
    # labels = copy.deepcopy(input_ids)
    # for label, source_len in zip(labels, model_inputs["input_ids_lens"]):
    #         label[:source_len] = -100
    # data_dict = dict(input_ids=input_ids, labels=labels)

    # model_inputs = tokenizer(input_text, truncation=True, padding='max_length', max_length=128, pad_to_max_length = True)
